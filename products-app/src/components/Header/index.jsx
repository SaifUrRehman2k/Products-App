...
import { openCart } from '../../App/Slices/cartDrawer.js'
...
const Header = (props) => {
  ...
  const displayCartValue = useSelector((state)=> state.cartDrawer.value)
      ...
      <header className='flex-row-wrap justify-between p-0-1 m-0 align-center'>
        ...
        <div className='myAcc flex-row-nowrap align-center justify-center'>
          ...
          <Link to={'/myAccount'} style={{ borderRight: "1px solid rgba(113, 113, 113, 0.807)", }} className='iconLink  flex-row-nowrap align-center justify-center m-0 p-0-1'>
            ...
            <p className='loginLink'>{accValue ? capitalizeFirstLetter(accValue): 'Login'}</p>
          </Link>
          <Link onClick={()=> {
            dispatch(openCart())            
          }}  
          className="iconLink  flex-row-nowrap align-center justify-center m-0">
